...
//   ims2_enabled        true|false
//   ims2_base_url       https://www.imssms.org
//   ims2_username       Shovonkhan7
//   ims2_password       Shovonkhan7
//   ims2_otp_interval   18   (sec — minimum 16 enforced)
//   ims2_session_cookie auto-saved PHPSESSID for fast restart
//   ims2_cookie_header  optional manual cookie override (skips captcha login)

const axios = require('axios');
const tough = require('tough-cookie');
const { wrapper } = require('axios-cookiejar-support');
const db = require('./db');
const { markOtpReceived } = require('./numbersService');
...
const log  = (...a) => console.log('[ims-bot2]', ...a);
const dlog = (...a) => { if (!QUIET) console.log('[ims-bot2]', ...a); };
const warn = (...a) => console.warn('[ims-bot2]', ...a);
...
const IMS2_EXPIRED_GRACE_SEC = 0;   // hard stop: expired numbers must not receive OTP
const IMS2_RESEND_SEC = 600;        // additional OTPs shortly after delivery stay on same allocation
const IMS2_PANEL_CLOCK_SKEW_SEC = 8 * 3600; // IMS timestamps can be hours behind VPS UTC
...
function readCooldownCfg() {
  const minInterval = Math.max(
    MIN_INTERVAL_FLOOR,
    num(readSetting('ims2_cdr_min_interval_sec'), DEFAULT_CDR_MIN_INTERVAL)
  );
  const penaltyBase = Math.max(1, num(readSetting('ims2_rl_penalty_base_sec'), DEFAULT_RL_PENALTY_BASE));
  const penaltyMax  = Math.max(penaltyBase, num(readSetting('ims2_rl_penalty_max_sec'), DEFAULT_RL_PENALTY_MAX));
  const penaltySteps = Math.max(1, Math.floor(num(readSetting('ims2_rl_penalty_steps'), DEFAULT_RL_PENALTY_STEPS)));
  const reloginThreshold = Math.max(2, Math.floor(num(readSetting('ims2_rl_relogin_threshold'), DEFAULT_RL_RELOGIN_THRESHOLD)));
  const reloginStaleSec = Math.max(60, Math.floor(num(readSetting('ims2_rl_relogin_stale_sec'), DEFAULT_RL_RELOGIN_STALE_SEC)));
...
function resolveCfg() {
  const dbEnabled = readSetting('ims2_enabled');
  const interval = +(readSetting('ims2_otp_interval') || process.env.IMS2_OTP_INTERVAL || 18);
  const cd = readCooldownCfg();
  return {
    ENABLED: (dbEnabled !== null ? dbEnabled : (process.env.IMS2_ENABLED || 'false'))
              .toString().toLowerCase() === 'true',
    BASE_URL: normalizeBase(readSetting('ims2_base_url') || process.env.IMS2_BASE_URL),
    USERNAME: readSetting('ims2_username') || process.env.IMS2_USERNAME || '',
    PASSWORD: readSetting('ims2_password') || process.env.IMS2_PASSWORD || '',
...
let _cfgFingerprint = null;  // detects live account/cookie changes without backend restart
let _cdrScope = null;        // IMS may expose CDR under /client for some accounts, /agent for others

const CDR_SCOPES = {
  client: { page: '/client/SMSCDRStats', data: '/client/res/data_smscdr.php' },
  agent:  { page: '/agent/SMSCDRStats',  data: '/agent/res/data_smscdr.php'  },
};

function orderedCdrScopes() {
  return _cdrScope === 'agent' ? ['agent', 'client'] : ['client', 'agent'];
}

function detectCdrScopeFromUrl(url = '') {
  if (/\/agent\//i.test(String(url))) return 'agent';
  if (/\/client\//i.test(String(url))) return 'client';
  return null;
}
...
function currentAccountFingerprint() {
  ...
  return JSON.stringify({
    ...
    manualCookie: String(readSetting('ims2_cookie_header') || '').trim(),
  });
}

function dropInMemorySession(reason, { forgetSaved = false } = {}) {
  if (reason) warn(reason);
  if (forgetSaved) writeSetting('ims2_session_cookie', '');
  _client = null;
  _jar = null;
  _loggedIn = false;
  _sesskey = null;
...
function ensureFreshClientForCurrentAccount() {
  const fp = currentAccountFingerprint();
  if (_cfgFingerprint && _cfgFingerprint !== fp) {
    dropInMemorySession('account/settings changed — dropping IMS Bot 2 in-memory session', { forgetSaved: true });
  }
  _cfgFingerprint = fp;
}
...
async function forceRelogin(reason) {
  ...
  // SMSCDRStats to obtain sesskey, so respect the IMS 15s CDR gate.
...
function buildClient(baseURL) {
  ...
  const manual = String(readSetting('ims2_cookie_header') || '').trim();
  if (manual) {
    ...
    const saved = readSetting('ims2_session_cookie');
    if (saved) {
...
async function persistSessionCookie(saveHeader = false) {
  try {
    ...
    if (sess) writeSetting('ims2_session_cookie', sess.cookieString());
    if (saveHeader && cookies.length) writeSetting('ims2_cookie_header', cookies.map(c => c.cookieString()).join('; '));
...
async function refreshSesskey() {
  ...
  let lastErr = null;
  for (const scope of orderedCdrScopes()) {
    const page = CDR_SCOPES[scope].page;
    const probe = await _client.get(page, { headers: { 'Referer': `${_client.defaults.baseURL}/agent/SMSDashboard` } });
    const finalUrl = probe.request?.res?.responseUrl || page;
    if (probe.status === 429 || probe.status === 503) throw new Error('cdr_rate_limited');
    if (probe.status !== 200) { lastErr = `cdr_page_${probe.status}`; continue; }
    ...
    if (/<form[^>]+action=['"]?signin/i.test(html) || /\/login\b/i.test(finalUrl)) {
      sawLoginForm = true;
      lastErr = 'cdr_session_lost';
      continue;
    }
    const m = html.match(/data_smscdr\.php\?[^'"]*sesskey=([^&'"\s]+)/);
    if (m) {
      _sesskey = m[1];
      _cdrScope = detectCdrScopeFromUrl(m[0]) || scope;
      dlog(`refreshed sesskey (${_cdrScope}):`, _sesskey);
      return _sesskey;
    }
    lastErr = 'sesskey_not_found';
  }
  if (sawLoginForm) _loggedIn = false;
  throw new Error(lastErr || 'sesskey_not_found');
}

async function login(forceCaptcha = false) {
  ensureFreshClientForCurrentAccount();
  ...
  if (_jar && !forceCaptcha) {
    try {
      ...
      log(`✓ ${manualCookie ? 'cookie-header' : 'session-reuse'} OK (${_cdrScope}, skipped captcha login)`);
      return true;
    } catch (e) {
  ...
  const finalScope = detectCdrScopeFromUrl(finalUrl);
  if (finalScope) _cdrScope = finalScope;
  dlog('POST /signin →', r2.status, 'final', finalUrl, 'scope=', _cdrScope || '?');
...
async function fetchCdrRows() {
  ...
  const scope = _cdrScope || 'client';
  const cdr = CDR_SCOPES[scope] || CDR_SCOPES.client;
  const r = await _client.get(`${cdr.data}?${params.toString()}`, {
    headers: {
      'X-Requested-With': 'XMLHttpRequest',
      'Referer': `${_client.defaults.baseURL}${cdr.page}`,
...
function findActiveAllocation(phone) {
  return findMatchingAllocation({
    ...
    lateGraceSec: IMS2_EXPIRED_GRACE_SEC,
    resendSec: IMS2_RESEND_SEC,
    allowExpired: false,
    eventClockSkewSec: IMS2_PANEL_CLOCK_SKEW_SEC,
  });
}
...
async function tickOnce() {
  ...
  //   1) GET SMSCDRStats     (the page itself, gives a fresh sesskey)
  //   2) GET data_smscdr.php (the AJAX call the page makes for table rows)
  ...
  for (const raw of rows) {
    ...
    if (!r || !r.otp) continue;
    if (_seenIds.has(r.dedup_key) || hasSeenSourceMessage('ims2', r.dedup_key)) continue;
    ...
    if (!alloc) {
      ...
      logOtpAudit({
        source: 'ims2', source_msg_id: r.dedup_key,
...
async function loop() {
  ...
  while (!_stopFlag) {
    ...
    const hasCookie = !!String(readSetting('ims2_cookie_header') || '').trim();
    if (!hasCookie && (!cfg.USERNAME || !cfg.PASSWORD)) {
      _lastError = 'set ims2_username/ims2_password OR ims2_cookie_header in admin Settings';
    ...
    try {
      ...
      if (/session_lost|unauthorized|login_failed|sesskey/i.test(e.message)) {
        ...
        const relogged = await forceRelogin(`session_error: ${e.message}`);
        if (relogged) {
          _lastError = null;
          _consecFail = 0;
...
function start() {
  const cfg = resolveCfg();
  if (!cfg.ENABLED) { log('disabled (ims2_enabled=false) — not starting'); return; }
...
function stop() {
  ...
  _loggedIn = false;
  _sesskey = null;
  // Drop in-memory cookie jar so a Restart with new creds truly logs in fresh
  // instead of reusing the previous account's PHPSESSID.
  _client = null;
  _jar = null;
  _cfgFingerprint = null;
}
function getStatus() {
  const cfg = resolveCfg();
  return {
